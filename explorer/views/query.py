...
class PlayQueryView(PermissionRequiredMixin, ExplorerContextMixin, View):
    ...
    def get(self, request):
        ...
        if url_get_log_id(request):
            ...
            c = log.connection or ""
            query = Query(sql=log.sql, title=_("Playground"), connection=c)
    ...
    def post(self, request):
        ...
        # pass sql blacklist if user is superuser
        current_user = str(request.user)
        if request.user.is_superuser:
            passes_blacklist = True
            logger.debug(
                f"current user: {current_user}, pass sql blacklist if user is superuser: {passes_blacklist}"
            )
        # pass sql blacklist if user in EXPLORER_USER_WHITELIST
        user_whilelist = app_settings.EXPLORER_USER_WHITELIST
        for user in user_whilelist:
            if current_user == user:
                # logger.debug(f"IF - current user: {current_user}, user in whitelist: {user}")
                passes_blacklist = True
                logger.debug(
                    f"current user: {current_user}, pass sql blacklist if user in EXPLORER_USER_WHITELIST: {passes_blacklist}"
                )
                break
    ...
    def render(self):
        return self.render_template(
            "explorer/play.html", {"title": _("Playground"), "form": QueryForm()}
        )
...
class QueryView(PermissionRequiredMixin, ExplorerContextMixin, View):
    ...
    def get(self, request, query_id):
        ...
        show = url_get_show(request)
        rows = url_get_rows(request)
        vm = query_viewmodel(request, query, form=form, run_query=show, rows=rows)
        fullscreen = url_get_fullscreen(request)
